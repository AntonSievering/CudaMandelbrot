...
void MandelbrotIntrinsics(const olc::vi2d& pix_tl, const olc::vi2d& pix_br, const olc::vd2d& frac_tl, const olc::vd2d& frac_br, const int iterations, size_t* pIterations)
{
	...
	for (y = pix_tl.y; y < pix_br.y; y++)
	{
		...
		repeat:
			...
			pIterations[y_offset + x + 0] = int(_n.m256i_i64[3]);
			pIterations[y_offset + x + 1] = int(_n.m256i_i64[2]);
			pIterations[y_offset + x + 2] = int(_n.m256i_i64[1]);
			pIterations[y_offset + x + 3] = int(_n.m256i_i64[0]);
